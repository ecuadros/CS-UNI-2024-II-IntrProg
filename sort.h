...
template <typename T>
int partition(T arr[], int low, int high, bool ascendente) {
    ...
    while (true) {
        ...
        {
            // Por la izq deben estar los mayores
            while (i <= j && arr[i] >= pivot)   i++;
            // Por la der deben estar los menores
            while (i <= j && arr[j] < pivot)    j--;
        }
...
template <typename T>
void QuickSort(T arr[], int low, int high, bool ascendente = true) {
    if (low < high) {
